...
double MeanEnergy(){
  ...
  for(int nx=1; nx<=NX-1;nx++){
    for(int ny=0; ny<=NY-1;ny++) Energy+=spin[nx][ny]*spin[nx+1][ny+1];
  }
  Energy=-J*Energy/(NX*NY);
...
double MeanEnergySq(){
  ...
  double EnergySq=0;
  for(int nx=1; nx<=NX-1;nx++){
    for(int ny=0; ny<=NY-1;ny++) Energy+=pow(spin[nx][ny]*spin[nx+1][ny+1],2);
  }
  EnergySq=J*J*Energy/(NX*NY);
...
double C(double T){
  ...
  C=(MeanEnergySq()-MeanEnergy()*MeanEnergy())/(T*T*NX*NX*NY*NY);
  return C;
}
...
void DisplayLattice(double T) {
  ...
  printf("T = %lf:   magnetization <sigma> = %lf Energy=%lf C = %0.11lf\n", T, Magnetization(),MeanEnergy(),C(T));
...
int main() {
  ...
  for (itemp=ntemp; itemp>0; itemp--) {
    ...
    fprintf(output, "%lf %lf %lf %0.11lf\n", T, (double)nmag/ntotal, MeanEnergy(),C(T));
