...
void Integral5d(long ne6=1000, long skip=0){
  gStyle->SetOptStat(0);
  // setup generators
  TRandom3 tr3(0);  
  SobolN sob(5,skip);
  ...
  long NTRIALS = 1000*1000*ne6;//1000*1000*ne6
  ...
  const double V5D = (8*PISQ/15/32-pow(sqrt(2)/2,5)/32); //TMath::Abs Volume of positive hyperquadrant // hyper cube inside has each side 2/sqrt(2)
  const double VBOX = 1;

  cout << "Study Integrals using " << ne6 << " million points" << endl;
  ...
  for (long n=MINTRIALS; n<=NTRIALS; n++){
    ...
    if (n==showat){
      ...
      if (n<1000*1000*2000){//1000*1000*100
	...
	if (n<1000*1000*2000 || j<2)//1000*1000*100
	  graphs[j]->SetPoint(graphs[j]->GetN(),n,err);
	//cout << err <<endl;
      }
    }
  }
  ...
  tc->SetLogy();
  tc->SetLogx();
  TH2F *hax=new TH2F("hax",";Number of samples;Relative error",
		     10,MINTRIALS-1,NTRIALS,10,1e-1,1.2);//,1e-8,2
  ...
  tl->AddEntry(graphs[0],"Uniform random","l");
  tl->AddEntry(graphs[3],"Subrandom","l");
  tl->AddEntry(graphs[1],"Sobol","l");
  tl->Draw();
  tc->Update();
  
  tc->Print("sphere5d.pdf");
  tc->Print("sphere5d.png");
...
int main(int argc, char **argv) {
  int ne6=1000;  // 1000 million samples
  long skip=0;
