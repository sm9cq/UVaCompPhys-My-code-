...
double trapez (double (*f)(double x), unsigned npts, double min, double max) {
  ...
  interval = ((max-min) / (npts-1));
  for (unsigned n=2; n<npts; n++){           	// sum the midpoints 
    x    = interval * (n-1);      
    sum += f(x)*interval;
  }
...
double simpson (double (*f)(double x), unsigned npts, double min, double max){  
  ...
  for (unsigned n=2; n<npts; n+=2){                // loop for odd points  
    x = interval * (n-1);
    sum += 4 * f(x);
  }
  for (unsigned n=3; n<npts; n+=2){                // loop for even points  
    x = interval * (n-1);
    sum += 2 * f(x);
  }   
  sum +=  f(min) + f(max);	 	// add first and last value           
  sum *= interval/3.;        		// then multiply by interval/scale
...
void GaussInt::Init(int npoints){
  ...
  for (int i=0; i<npoints*2; i+=2){
    ...
    //cout << *weight.rbegin() << " " << *lroots.rbegin() << endl;
  }
#endif  
}
